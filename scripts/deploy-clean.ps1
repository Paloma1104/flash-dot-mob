...
if (-not $privateKey -or $privateKey -eq "YOUR_BACKEND_WALLET_PRIVATE_KEY_HERE") {
    ...
    Write-Host "1. Create a new MetaMask wallet" -ForegroundColor White
    Write-Host "2. Get testnet MON from: https://faucet.monad.xyz" -ForegroundColor White
    Write-Host "3. Export private key from MetaMask" -ForegroundColor White
    Write-Host "4. Add to .env: PRIVATE_KEY=0x..." -ForegroundColor White
    Write-Host ""
    exit 1
}
...
if ($deploySuccess) {
    ...
    Write-Host "2. Update .env file with:" -ForegroundColor White
