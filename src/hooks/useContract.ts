...
export function useContract() {
  ...
  const isDropClaimed = useCallback(
    async (dropId: string): Promise<boolean> => {
      ...
      try {
        const dropIdBytes = keccak256(
          encodeAbiParameters(parseAbiParameters("string"), [dropId]),
        );

        const claimed = await publicClient.readContract({
          address: contracts.flashMob!,
  ...
  const getTokenBalance = useCallback(
    async (userAddress: `0x${string}`): Promise<string> => {
      if (!contracts.apToken) {
        return "0";
      }
  ...
  const encodeClaimData = useCallback((params: ClaimParams): `0x${string}` => {
    const dropIdBytes = keccak256(
      encodeAbiParameters(parseAbiParameters("string"), [params.dropId]),
    );
  ...
  const createClaimMessageHash = useCallback(
    (
      ...
      return keccak256(
        encodeAbiParameters(
          parseAbiParameters(
            "bytes32,address,uint256,int256,int256,uint256,uint256,uint256",
          ),
          [
            dropIdBytes,
            userAddress,
